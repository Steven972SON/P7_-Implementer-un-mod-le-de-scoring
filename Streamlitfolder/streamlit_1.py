...
fig1 = exp.bar(data_group, x='ID_CLIENT', y='AMT_GOODS_PRICE', width=800, height=400, title="Montants demandés par les clients similaires",
               labels={"AMT_GOODS_PRICE": "Montant demandé", "ID_CLIENT": "Identifiants client"})
...
fig2 = exp.histogram(data_raw_group, x='NAME_FAMILY_STATUS', width=800, height=400, title="État civil des clients similaires",
                     labels=dict(y='Nombre de clients', NAME_FAMILY_STATUS='État civil'))
fig2.update_xaxes(type='category')
st.write(fig2)

fig3 = exp.histogram(data_raw_group, x='NAME_INCOME_TYPE', width=800, height=400, title="Type de revenus des clients similaires",
                     labels=dict(y='Nombre de clients', NAME_INCOME_TYPE='Type de revenus'))
fig3.update_xaxes(type='category')
st.write(fig3)

fig4 = exp.histogram(data_raw_group, x='OCCUPATION_TYPE', width=800, height=400, title="Professions des clients similaires",
                     labels=dict(y='Nombre de clients', OCCUPATION_TYPE='Profession'))
fig4.update_xaxes(type='category')
st.write(fig4)

fig5 = exp.histogram(data_raw_group, x='ORGANIZATION_TYPE', width=800, height=400, title="Entreprises des clients similaires",
...
def infosup():
    ...
    for i in range(0, len(mask_copy['ID_CLIENT'])):
        if mask_copy.loc[i, 'ID_CLIENT'] == user:
            mask_copy.loc[i, 'couleur'] = 'Sélectionné'
        else:
            mask_copy.loc[i, 'couleur'] = 'Autre'

    mask_copy.sort_values(by='ID_CLIENT')

    fig7 = exp.bar(mask_copy, x='ID_CLIENT', y='AMT_GOODS_PRICE', color="couleur",
                   width=800, height=400, title="Montants demandés par chaque client")
    fig7.update_xaxes(type='category')
    fig7.add_hline(y=mask_copy.AMT_GOODS_PRICE.mean(), line_dash="dash", line_color="green")
    # st.write(fig7)

    if predict == '[0]':
        mask_copy_user = mask_copy[mask_copy['ID_CLIENT'] == user]
        if mask_copy_user.FLAG_DOCUMENT_3.values <= 0:
            st.write("Des informations vous concernant sont manquantes ou non renseignées au niveau du document 3.")
        elif mask_copy_user.EXT_SOURCE_1.values <= 0:
            st.write("Des informations sur vos prêts antérieurs sont manquants. Voir EXT_SOURCE_1")
        elif mask_copy_user.EXT_SOURCE_2.values <= 0:
            st.write("Des informations sur vos prêts antérieurs sont manquants. Voir EXT_SOURCE_2")
        elif mask_copy_user.EXT_SOURCE_3.values <= 0:
            st.write("Des informations sur vos prêts antérieurs sont manquants. Voir EXT_SOURCE_3")
        else:
            st.write("Observez les autres variables!")

    shap_user.rename(columns={0: 'Valeur'}, inplace=True)
    shap_user['colors'] = np.where(shap_user['Valeur'] < 0, 'red', 'blue')
    fig9 = exp.bar(shap_user, y=shap_user.index, x='Valeur', width=800, height=1000, color='colors',
                   title="Implication des variables à la décision d'acceptation ou de rejet au prêt",
                   labels=dict(index='Variable', x='Valeur'))
    st.write(fig9)

    var_choose = st.selectbox("Choisir une variable à observer ", options=mask_copy.columns.tolist())
    fig8 = exp.bar(mask_copy, x='ID_CLIENT', y=var_choose, color='couleur',
                   width=800, height=400, title="Valeur de la variable d'intérêt par client")
